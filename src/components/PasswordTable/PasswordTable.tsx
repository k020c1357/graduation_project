...
function RenderTableItems({
  ...
  const openShowModal = () => (
    setShowViewModal(true), setViewModalItem(password)
  );
    ...
    >
      ...
      <Menu>
        ...
        <MenuList color="black">
          <MenuItem fontWeight="bold" onClick={() => handleEdit(password)}>
            Edit
          </MenuItem>
          <MenuItem
            fontWeight="bold"
...
export function PasswordTableComponent({
  ...
  const [passwordToDelete, setPasswordToDelete]: IpasswordToDelete =
    React.useState(null);
  const [showActionModal, setActionModal] = React.useState(false);
  const [showViewModal, setShowViewModal] = React.useState(false);
  const [viewModalItem, setViewModalItem]: IviewModalItem =
    React.useState(null);
  ...
  const handleEdit = (password: Password) => (
    setSelectedPassword(password), setShowModal(true)
  );
  const handleDelete = (password: Password) => (
    setPasswordToDelete(password), setActionModal(true)
  );
  const handleCancelDelete = () => (
    setPasswordToDelete(null), setActionModal(false)
  );
  const closeShowModal = () => (
    setShowViewModal(false), setViewModalItem(null)
  );
  ...
  return (
    <>
      <Box
        display="grid"
        gridTemplateColumns={isLargerThan1280 ? "repeat(2, 1fr)" : "1fr"}
        className="md-column"
        p={1}
      >
