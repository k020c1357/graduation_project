...
function SidebarTags(props: Props) {
  ...
  const handleDeleteTag = (tag: Tag) => {
    ...
    createNotificationDispatch({
      type: "success",
      message: `${tag.name} tag removed successfully`,
    });
  ...
  return (
    ...
    >
      {TAGS_STATE.tags &&
        TAGS_STATE.tags.map((item: Tag) => (
          <Box
            key={"__tag_item__" + item.id}
            _hover={{ backgroundColor: "rgba(255, 255, 255, 0.03)" }}
            cursor="pointer"
            className={`app-box-item ${
              TAGS_STATE.selectedTag.id === item.id ? "selected" : ""
            }`}
            display="flex"
...
const mapDispatchToProps = (dispatch: Dispatch) => {
  return {
    ...
    createNotificationDispatch: (notification: Notification) =>
      dispatch(createNotification(notification)),
  };
};

export const SidebarTagsGroupComponent = connect(
  mapStateToProps,
  mapDispatchToProps
)(SidebarTags);
